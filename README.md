...
- **画像サムネイルのグリッド表示** (LazyVGrid、メモリキャッシュ、自動リトライ)
- **画像詳細画面** (display サイズ画像 + メタデータ: ファイルサイズ・撮影日時・画像 px サイズ・回転角度・レーティング 等)
...
- 転送キューの永続化 (SwiftData)
- 自動再接続 (指数バックオフ)
- 画像オリジナルダウンロード (Files / Photos 連携、必要になり次第)
...
- **Phase 4**: 機能拡張 (リモート撮影、設定変更、ライブビュー、アップロード、画像ダウンロード等)
