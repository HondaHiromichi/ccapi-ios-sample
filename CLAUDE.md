...
└── ccapi-ios-sample/
    ├── ccapi_ios_sampleApp.swift              # App エントリ (AppSettings 注入)
    ...
    └── Services/
        ...
        ├── CCAPIClient.swift                  # URLSession ラッパー
...
- [x] Phase 1: 実機検証 (CCAPI 有効化、エンドポイント疎通、API 仕様の確定) — 同時接続挙動のみ Phase 2 で追検予定
- [ ] Phase 2: MVP (進行中)
  - [x] Xcode プロジェクト雛形
  ...
  - [x] コンテンツ一覧取得・表示 (フォルダ・ファイル数・サンプルファイル名)
  - [ ] 画像サムネイルグリッド UI
  - [ ] 画像詳細・中サイズ表示
  - [ ] 画像オリジナルダウンロード
- [ ] Phase 3: 安定化 (リトライ / 接続監視 / 自動再接続 / イベントポーリング / 転送キュー永続化)
